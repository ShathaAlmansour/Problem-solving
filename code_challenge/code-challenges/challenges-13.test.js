...
const standardizePhoneNumbers = (arr) => {
  ...
  arr.map((element, value) => {
    array[value] = "" + element.match(/\d+/g);

    array[value] = array[value].split(",").join("");
  });
...
const unenrollBrook = (arr) => {
  return arr.map((value) => {
    return value.filter((element) => {
      if (!element.includes("Brook")) return element;
    });
  });
};
...
const sortByDay = (arr) => {
  ...
  daysOfWeek.forEach((day, i) => {
    ...
    arr.forEach((event) => {
      if (event.includes(day)) values[i].push(event);
    });
  });
